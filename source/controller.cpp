...
VSTGUI::CView* Controller::createCustomView(VSTGUI::UTF8StringPtr name,const VSTGUI::UIAttributes& a,const VSTGUI::IUIDescription* description,VSTGUI::VST3Editor* e){
    ...
    if(std::strcmp(name,"VULeft")==0)return new HardwareVUMeter(r,e,kParamMeterL,description?description->getBitmap("MixVUAtlasA"):nullptr,description?description->getBitmap("MixVUAtlasB"):nullptr);
    if(std::strcmp(name,"VURight")==0)return new HardwareVUMeter(r,e,kParamMeterR,description?description->getBitmap("MixVUAtlasA"):nullptr,description?description->getBitmap("MixVUAtlasB"):nullptr);
