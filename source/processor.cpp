...
inline double tubeAutoGain(int type, double amount) {
    ...
    double m=2.6; switch(type){case kTube12AU7:m=1.5;break;case kTube12AT7:m=2.6;break;default:m=3.6;break;} return dbToGain(-m*a*a);
}
inline double tapeAutoGain(double amount){const double a=std::clamp(amount,0.0,1.0);return a<=0.0?1.0:dbToGain(1.2*a*a);}
