...
int main(){
    try{
        const std::vector<Case> cases={
            ...
            {"Quality",MixEngine::kParamQuality,0.0,1.0,
        ...
        if(!ok){
            std::cerr<<"FAILED: at least one automation parameter depends on host block partitioning\n";
            return 1;
        }
        std::cout<<"PASSED: standard VST3 sample-accurate automation matrix\n";
        return 0;
    }catch(int c){std::cerr<<"Sample-accuracy setup FAIL "<<c<<"\n";return c;}
    catch(...){std::cerr<<"Sample-accuracy unknown exception\n";return 90;}
}
